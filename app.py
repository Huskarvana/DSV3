...
def fetch_contextual_articles(query, max_results=5):
    params = {"q": query, "pageNumber": "1", "pageSize": str(max_results), "autoCorrect": "true"}
...
def analyser_article(row):
    ...
    positif = ["succès", "innovant", "croissance", "récompense", "victoire"]
    negatif = ["baisse", "déclin", "problème", "crise", "perte", "licenciement"]
    contenu = row['contenu'].lower()
    if any(word in contenu for word in positif):
        ton = "Positif"
    elif any(word in contenu for word in negatif):
        ton = "Négatif"
    else:
        ton = "Neutre"
...
if st.button("🔍 Lancer la veille maintenant"):
    st.info("Chargement des articles... Cela peut prendre quelques secondes.")
    try:
